...
class Datakrs_model extends BF_Model {
	...
	public function krsbytahunanakademik($tahun = "",$filfakultas = "",$filljurusan = "",$mhs="",$status = "",$angkatan = "")
	{
		...
		if($tahun!=""){
			$this->db->where('datakrs.tahun_akademik',$tahun);
		}
		if($filljurusan!=""){
		...
		$this->db->group_by('tahun_akademik,mahasiswa');  
		return parent::find_all();
	}
	public function distinctmahasiswa($tahun = "",$filfakultas = "",$filljurusan = "",$mhs="",$status = "",$angkatan = "")
	{
		
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.semester,sum(sks) as jml_sks,mahasiswa,nama_mahasiswa,tahun_akademik');
		}
		$this->db->where('datakrs.tahun_akademik',$tahun);
		$this->db->where('mastermahasiswa.kode_prodi',$filljurusan);
		$this->db->where('mastermahasiswa.kode_fakultas',$filfakultas);
	...
	public function findbyangkatan($tahun_ajaran="",$angkatan="")
	{
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.semester,sum(sks) as jml_sks,sum(nilai_angka) as ipk,mahasiswa,nama_mahasiswa');
		...
		if($angkatan!=""){
			$this->db->where('datakrs.mahasiswa like "'.$angkatan.'%"');
		}
		$this->db->join('mastermahasiswa', 'mastermahasiswa.nim_mhs = datakrs.mahasiswa', 'inner');
		$this->db->group_by('semester,mahasiswa');  
		return parent::find_all();
	...
	public function find_all($sms="",$kode_mk="",$mhs="")
	{
		...
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.*,datakrs.kode_mk,nama_mk,nama_dosen,nim_mhs,nama_mahasiswa');
		}
		//$this->db->join('mastermatakuliah mm', 'mm.kode_mata_kuliah=datakrs.kode_mk', 'left');
		if($sms!=""){
			$this->db->where('datakrs.semester',(int)$sms);
		...
		$this->db->join('mastermahasiswa', 'mastermahasiswa.nim_mhs = datakrs.mahasiswa', 'left');
		$this->db->join('jadwal', 'jadwal.id = datakrs.kode_jadwal', 'inner');
		$this->db->join('masterdosen', 'jadwal.kode_dosen = masterdosen.id', 'left');
	...
	function update_nama_matakuliah($kode_mk, $nama_mk, $prodi){
        ...
        $this->db->where('mahasiswa in (select nim_mhs from simak_mastermahasiswa where kode_prodi = "'.$prodi.'")');
	...
	public function cek_nama_bykode($kode="")
	{
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.nama_mk,kode_mk');
		}
	...
	public function find_krs($sms="",$kode_mk="",$mhs="")
	{
		...
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.*,datakrs.kode_mk,nama_mk,nama_dosen,namadosen,nim_mhs,nama_mahasiswa');
		}
		//$this->db->join('mastermatakuliah mm', 'mm.kode_mata_kuliah=datakrs.kode_mk', 'left');
		//die($sms." ini");
		if($sms!=""){
	...
	public function find_krsprint($sms="",$mhs="")
	{
	...
	public function find_distinct($sms="",$kode_mk="",$mhs="")
	{
		...
		//$this->db->where('simak_mastermahasiswa.kode_prodi = mm.kode_prodi');
		$this->db->join('mastermahasiswa', 'mastermahasiswa.nim_mhs = datakrs.mahasiswa', 'left');
		//$this->db->join('masterdosen', 'masterdosen.nidn = datakrs.kode_dosen', 'inner');
		$this->db->join('jadwal', 'jadwal.id = datakrs.kode_jadwal', 'left');
		$this->db->join('masterdosen', 'jadwal.kode_dosen = masterdosen.id', 'left');
		$this->db->order_by("datakrs.semester","asc");
		$this->db->distinct();
	...
	public function find_bysenester($sms="",$mhs="")
	{
		...
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.*,datakrs.kode_mk,nama_mk,namadosen,nama_dosen,nim_mhs,nama_mahasiswa,hari,jam,materi,prodi,jadwal.id as kode_jadwal');
		}
		//$this->db->join('mastermatakuliah mm', 'mm.kode_mata_kuliah=datakrs.kode_mk', 'left');
		if($sms!=""){
			$this->db->where('datakrs.semester',(int)$sms);
	...
	public function find_absen_dosen($ta="",$jurusan="",$kode_mk="",$dosen="",$sms="",$kelas="")
	{
		...
		if($dosen != ""){
			$this->db->where('masterdosen.id',$dosen);
		}
		if($sms!=""){
			$this->db->where('datakrs.semester',(int)$sms);
		} 
	...
	public function find_bytahun($ta = "",$kode_prodi = "")
	{
		
		if(empty($this->selects))
		{
			$this->select($this->table_name .'.*,datakrs.kode_mk,nama_mk,nama_mahasiswa,kode_prodi');
		}
	...
	public function find_transkip($nim='',$mk="",$nilai="")
	{
		...
		$this->db->where("nilai_huruf is not null");
		$this->db->where("nilai_huruf != ''");
	...
	public function find_konsentrasi($nim='')
	{
		if(empty($this->selects))
		{
			$this->select('mastermatakuliah.nama_konsentrasi as nama_konsentrasi');
		}
		...
		$this->db->where("nilai_huruf is not null");
		$this->db->where("nilai_huruf != ''");
		$this->db->where("konsentrasi","Y");
		$this->db->order_by("datakrs.id","desc");
		$this->db->distinct();  
		$this->db->join('mastermatakuliah', 'datakrs.kode_mk = mastermatakuliah.kode_mata_kuliah', 'inner'); 
		return parent::find_all();
	}
	public function countmhs_bydosen($ta="",$jurusan="",$kode_mk="",$dosen="",$sms="",$kelas="")
	{
		
		if(empty($this->selects))
		{
			$this->select('count(nim_mhs) as jmlmahasiswa');
		}
		//$this->db->join('mastermatakuliah mm', 'mm.kode_mata_kuliah=datakrs.kode_mk', 'left');
		if($jurusan != ""){
			$this->db->where('mastermahasiswa.kode_prodi',$jurusan);
		...
		if($sms!=""){
			$this->db->where('jadwal.semester',(int)$sms);
		} 
	...
	public function getjmlmahasiswa($tahunakademik = "",$kode_prodi = "")
	{
		if (empty($this->selects))
		{
			$this->select('count(distinct mahasiswa) as jml_mahasiswa');
		}
		if ($kode_prodi != "")
		{
			$this->db->where('kode_prodi',$kode_prodi);
		...
		if ($tahunakademik != "")
		{
			$this->db->where('tahun_akademik',$tahunakademik);
		} 
		$this->db->join('mastermahasiswa', 'mastermahasiswa.nim_mhs = datakrs.mahasiswa', 'inner');
		return parent::find_all();
