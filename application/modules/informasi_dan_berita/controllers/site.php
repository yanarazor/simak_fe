...
class site extends Admin_Controller
{
	...
	public function __construct()
	{
		...
		$this->auth->restrict('Informasi_dan_Berita.Site.View');
		...
		$this->load->model('kategori_berita/kategori_berita_model', null, true);
		$kategoris = $this->kategori_berita_model->find_all();
	...
	public function index()
	{
		...
		Template::set('toolbar_title', '&nbsp;&nbsp; Pengelolaan Informasi dan Berita');
		Template::render();
	}
	...
	{
		$this->auth->restrict('Informasi_dan_Berita.Site.Create');
		...
		if (isset($_POST['save']))
		{
			...
			if (isset($_FILES['file_upload']) && is_array($_FILES['file_upload']) && $_FILES['file_upload']['error'] != 4)
			{
				...
				$uploadData = handle_upload('file_upload',$this->settings_lib->item('site.pathphotoartikel'));
			...
			if ($insert_id = $this->save_informasi_dan_berita($uploadData))
			{
				...
				redirect(SITE_AREA .'/site/informasi_dan_berita');
			}
			else
			{
	...
	public function edit()
	{
		...
		if (isset($_POST['save']))
		{
			$this->auth->restrict('Informasi_dan_Berita.Site.Edit');
			$this->load->helper('handle_upload');
			$uploadData = array();
			$upload = true;
			if (isset($_FILES['file_upload']) && is_array($_FILES['file_upload']) && $_FILES['file_upload']['error'] != 4)
		...
		{
			$this->auth->restrict('Informasi_dan_Berita.Site.Delete');
