	...
	<?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
		<fieldset>
			...
			<div class="control-group <?php echo form_error('datakrs_kode_mk') ? 'error' : ''; ?>">
				 ...
				 <div class='controls'>
					 <select name="datakrs_kode_mk" id="datakrs_kode_mk" class="chosen-select-deselect" style="width:300px;">
						 <option value=""></option>
			...
			<div class="form-actions">
				<input type="submit" name="save" class="btn btn-primary" value="<?php echo lang('datakrs_action_create'); ?>"  />
				<?php echo lang('bf_or'); ?>
				<?php echo anchor(SITE_AREA .'/krs/krs_mahasiswa', lang('datakrs_cancel'), 'class="btn btn-warning"'); ?>
...
<script type="text/javascript">	  
	$("#tahun_akademik").change(function(){
		...
		$("#datakrs_kode_mk").empty().append("<option>loading...</option>"); //show loading...
		...
		$.getJSON(json_url,function(data){
			$("#datakrs_kode_mk").empty(); 
			if(data==""){
				$("#datakrs_kode_mk").append("<option value=\"\">Pilih Matakuliah </option>");
			}
			else{
				$("#datakrs_kode_mk").append("<option value=\"\">-- Pilih Matakuliah --</option>");
				for(i=0; i<data.id.length; i++){
					$("#datakrs_kode_mk").append("<option value=\"" + data.id[i]  + "\">" + data.nama_mata_kuliah[i] +"</option>");
				}
			} 
		}); 
		return false;
