                ...
                <td>
                	<select name="kode_mk" id="kode_mk" class="chosen-select-deselect">
						 ...
						 <?php foreach($matakuliahs as $record):?>
							  <option value="<?php echo $record->id?>" <?php if(isset($kode_mk))  echo  ($record->id==$kode_mk) ? "selected" : ""; ?>><?php echo $record->nama_mata_kuliah; ?></option>
						  <?php endforeach;?>
						 <?php endif;?>
					 </select>
                </td> 
	...
	<?php echo form_open($this->uri->uri_string()); ?>
		<table class="table table-striped">
			...
			<tbody>
				...
				<?php if ($can_edit) : ?>
					<td><?php echo anchor(SITE_AREA . '/krs/datakrs/edit/' . $record->id, '<span class="icon-pencil"></span>' .  $record->kode_mk); ?></td>
				<?php else : ?>
					<td><?php e($record->kode_mk); ?></td>
				<?php endif; ?>
					<td><?php e($record->sks) ?></td>
					 
					<td><?php e($record->kode_dosen) ?></td>
