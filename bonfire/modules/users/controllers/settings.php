...
class Settings extends Admin_Controller
{
	...
	public function uploadfoto($user_id='')
	{
	...
	function savefoto(){
		...
		$id_log = $this->input->post('id_log');
		 ...
		 if (isset($_FILES['userfile']) && is_array($_FILES['userfile']) && $_FILES['userfile']['error'] != 4)
		 {
			...
			$uploadData = handle_upload('userfile',$this->settings_lib->item('site.pathphotomahasiswa'));
			 if (isset($uploadData['error']) && !empty($uploadData['error']))
			 {
			 	$tipefile=$_FILES['userfile']['type'];
			 	//$tipefile = $_FILES['userfile']['name'];
				 $upload = false;
				 log_activity($this->auth->user_id(), 'Gagal : '.$uploadData['error'].$tipefile.$this->input->ip_address(), 'users');
			 }else{
			 	$namafile = $uploadData['data']['file_name'];
                log_activity($this->auth->user_id(), 'upload foto : ' . $id_log . ' : ' . $this->input->ip_address(), 'users');
			 }
		 }else{
		 	die("File tidak ditemukan");
		 	log_activity($this->auth->user_id(), 'File tidak ditemukan : ' . $this->input->ip_address(), 'users');
		 } 	
		if ($namafile != "")
		{
			$data = array(
			...
			$datamhs = array(
			'photo_mahasiswa'	=> $namafile
			);
			$this->mastermahasiswa_model->update_where("nim_mhs",$this->current_user->nim, $datamhs);
			}
		}
	...
	public function edit($user_id='')
	{
		...
		//die($nim."ini");
		$datamahasiswa = $this->mastermahasiswa_model->get_by_nim($nim);
		//print_r($datamahasiswa);
		//die($datamahasiswa[0]->id." ini");
		Template::set('mastermahasiswa', $datamahasiswa);
	...
	private function save_user($type='insert', $id=0, $meta_fields=array(), $cur_role_name = '')
	{
		...
		elseif($this->input->post('role_id')=="11")
		{
		...
		elseif($this->input->post('role_id')=="11")
		{
			$nim = $this->input->post('kode_prodi');
			$data['nim'] = $nim;
		}
