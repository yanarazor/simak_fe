...
class Auth
{
	...
	public function login($login, $password, $remember=FALSE)
	{
		...
		$selects = 'id, email, username, users.role_id, users.deleted, users.active, banned, ban_message, password_hash, password_iterations, force_password_reset,nim,foto';
		...
		if ($this->check_password($password, $user->password_hash))
		{
			...
			$this->setup_session($user->id, $user->username, $user->password_hash, $user->email, $user->role_id, $remember,'', $user->username, $user->nim, $user->foto);
	 ...
	 */
	private function setup_session($user_id, $username, $password_hash, $email, $role_id, $remember=FALSE, $old_token=NULL,$user_name='',$nim="",$foto = "")
	{
